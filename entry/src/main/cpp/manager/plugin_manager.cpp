...
napi_value PluginManager::GetContext(napi_env env, napi_callback_info info)
{
    if ((nullptr == env) || (nullptr == info)) {
        LOGE("PluginManager get context failed");
        return nullptr;
    }
    ...
    if (napi_ok != napi_get_cb_info(env, info, &argCnt, args, nullptr, nullptr)) {
        LOGE("PluginManager get cb info failed");
    }
...
void PluginManager::RegisterLifecycle(napi_env env, napi_value exports, int64_t value)
{
    ...
    switch (value) {
        case APP_LIFECYCLE: {
            LOGI("PluginManager register life cycle: app lifecycle");
        ...
        case PAGE_LIFECYCLE: {
            LOGI("PluginManager register life cycle: page lifecycle");
            napi_property_descriptor desc[] = {
        ...
        default: {
            LOGE("PluginManager register life cycle: wrong type of value");
            break;
        }
    }
}
...
void PluginManager::Export(napi_env env, napi_value exports)
{
    ...
    if (OH_NATIVEXCOMPONENT_RESULT_SUCCESS != OH_NativeXComponent_GetXComponentId(nativeXComponent, idStr, &idSize)) {
        LOGE("PluginManager export OH_NativeXComponent_GetXComponentId fail");
        return;
    }
